  ...
  <div class="document-requirements mt-0 mx-10 mb-9" id="anchor-<?= $req['documentType'] ?>">
    ...
    <div class="upload-container mt-4 mx-6 mb-0">
      <?php if (!$hasDocument) { ?>
        ...
        <button type="button" class="upload-btn block bg-[#7213D0] border-2 border-[solid] border-[black] text-[white] rounded-xl text-base font-bold px-16 py-1 mx-0 my-4 hover:bg-[white] hover:text-[black] hover:cursor-pointer hover:[transition:0.3s]" data-id="<?= $req['documentType'] ?>">Upload</button>
      <?php } else { ?>
